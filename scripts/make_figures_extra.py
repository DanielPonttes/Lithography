...
for idx, tgt in enumerate(targets):
    ...
    ax.set_title(f'TC {idx+1} — target (cinza) + contorno mascara NeuralILT (ciano)')
    ax.axis('off')
    fig.tight_layout()
    ...
    axs[0].set_title(f'PVBand continuo  (max={pv_np.max():.3f})'); axs[0].axis('off')
    axs[1].imshow(hot_bin, cmap=ListedColormap(['#111111', '#FFD54F']))
    axs[1].set_title(f'Hotspots (PVBand > {THRESH}) — {n_hot} pixels')
    axs[1].axis('off')
...
ax.set_title('Distribuicao cumulativa de PVBand por testcase')
...
ax.set_ylabel('# violacoes EPE')
ax.set_title(f'EPE por testcase — queda de {np.sum(init_epe)} -> {np.sum(ft_epe)} violacoes')
